...
def filter_data(file_path, event_keywords=None, source_name=None, min_year=None, max_year=None,
                source_exact_match=False, and_search=False):
...
def validate_source_name(args):
    """--strictが指定されているが-sが指定されていない場合はエラー"""
    if args.source_exact_match and not args.source_name:
        print ("--strictを指定する場合は、-s も指定してください")
        exit(1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("-s", "--source", dest="source_name", type=str, default=None, help="原出典の検索ワード（部分一致）")
    parser.add_argument("--strict", dest="source_exact_match", action="store_true", help="原出典の完全一致検索を有効にする")
    ...
    filtered_data = filter_data(input_file_path, args.event_keywords, args.source_name, 
